...
class CDbColumnSchema extends CComponent{
	...
	public function init($dbType, $defaultValue){
		...
		if($defaultValue !== null) $this->extractDefault($defaultValue);
	}
	...
	protected function extractType($dbType){
		if(stripos($dbType, 'int') !== false && stripos($dbType, 'unsigned int') === false) $this->type = 'integer';
		elseif(stripos($dbType, 'bool') !== false) $this->type = 'boolean';
		elseif(preg_match('/(real|floa|doub)/i', $dbType)) $this->type = 'double';
	...
	protected function extractLimit($dbType){
		if(strpos($dbType, '(') && preg_match('/\((.*)\)/', $dbType, $matches)){
			...
			if(isset($values[1])) $this->scale = (int)$values[1];
		}
	}
	...
	public function typecast($value){
		...
		if($value === '' && $this->allowNull) return $this->type === 'string'?'':null;
		switch($this->type){
			case 'string':
				return (string)$value;
			case 'integer':
