...
class CPgsqlSchema extends CDbSchema{
	 ...
	 * @since 1.1.6
	 */
	public $columnTypes = array('pk'=>'serial NOT NULL PRIMARY KEY','string'=>'character varying (255)','text'=>'text','integer'=>'integer','float'=>'double precision','decimal'=>'numeric','datetime'=>'timestamp','timestamp'=>'timestamp','time'=>'time','date'=>'date','binary'=>'bytea','boolean'=>'boolean','money'=>'decimal(19,4)');
	...
	public function resetSequence($table, $value = null){
		if($table->sequenceName === null) return;
		$sequence = '"' . $table->sequenceName . '"';
		if(strpos($sequence, '.') !== false) $sequence = str_replace('.', '"."', $sequence);
		if($value !== null) $value = (int)$value;
		else $value = "(SELECT COALESCE(MAX(\"{$table->primaryKey}\"),0) FROM {$table->rawName})+1";
	...
	protected function loadTable($name){
		...
		if(is_string($table->primaryKey) && isset($this->_sequences[$table->rawName . '.' . $table->primaryKey])) $table->sequenceName = $this->_sequences[$table->rawName . '.' . $table->primaryKey];
	...
	protected function resolveTableNames($table, $name){
		...
		if($schemaName === self::DEFAULT_SCHEMA) $table->rawName = $this->quoteTableName($tableName);
...
EOD;
		...
		foreach($columns as $column){
			...
			if(stripos($column['adsrc'], 'nextval') === 0 && preg_match('/nextval\([^\']*\'([^\']+)\'[^\)]*\)/i', $column['adsrc'], $matches)){
				if(strpos($matches[1], '.') !== false || $table->schemaName === self::DEFAULT_SCHEMA) $this->_sequences[$table->rawName . '.' . $c->name] = $matches[1];
				else $this->_sequences[$table->rawName . '.' . $c->name] = $table->schemaName . '.' . $matches[1];
...
EOD;
		...
		foreach($command->queryAll() as $row){
			...
			if(isset($table->columns[$name])){
				...
				if($table->primaryKey === null) $table->primaryKey = $name;
				elseif(is_string($table->primaryKey)) $table->primaryKey = array($table->primaryKey,$name);
				else $table->primaryKey[] = $name;
			}
		}
	}
	...
	protected function findForeignKey($table, $src){
		...
		if(preg_match($pattern, str_replace('"', '', $src), $matches)){
			...
			foreach($keys as $i=>$key){
				...
				if(isset($table->columns[$key])) $table->columns[$key]->isForeignKey = true;
			}
		}
	}
...
EOD;
		...
		$names = array();
		foreach($rows as $row){
			if($schema === self::DEFAULT_SCHEMA) $names[] = $row['table_name'];
			else $names[] = $row['table_schema'] . '.' . $row['table_name'];
		}
		return $names;
	}
