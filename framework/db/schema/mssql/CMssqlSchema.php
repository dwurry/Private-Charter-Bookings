...
class CMssqlSchema extends CDbSchema{
	 ...
	 * @since 1.1.6
	 */
	public $columnTypes = array('pk'=>'int IDENTITY PRIMARY KEY','string'=>'varchar(255)','text'=>'text','integer'=>'int','float'=>'float','decimal'=>'decimal','datetime'=>'datetime','timestamp'=>'timestamp','time'=>'time','date'=>'date','binary'=>'binary','boolean'=>'bit');
	...
	public function resetSequence($table, $value = null){
		...
		else $value = (int)$this->getDbConnection()->createCommand("SELECT MAX([{$table->primaryKey}]) FROM {$table->rawName}")->queryScalar();
	...
	public function checkIntegrity($check = true, $schema = ''){
		...
		if(!isset($this->_normalTables[$schema])) $this->_normalTables[$schema] = $this->findTableNames($schema, false);
...
EOD;
	...
	protected function findColumns($table){
		...
		if(isset($table->schemaName)) $where[] = "t1.TABLE_SCHEMA='" . $table->schemaName . "'";
		...
		foreach($columns as $column){
			...
			if(is_array($table->primaryKey)) $c->isPrimaryKey = in_array($c->name, $table->primaryKey);
			else $c->isPrimaryKey = strcasecmp($c->name, $table->primaryKey) === 0;
			
			$c->isForeignKey = isset($table->foreignKeys[$c->name]);
			$table->columns[$c->name] = $c;
			if($c->autoIncrement && $table->sequenceName === null) $table->sequenceName = $table->name;
		}
		return true;
	}
	...
	protected function createColumn($column){
		...
		}elseif($column['DATA_TYPE'] == 'image' || $column['DATA_TYPE'] == 'text') $c->size = $c->precision = null;
		else $c->size = $c->precision = ($column['CHARACTER_MAXIMUM_LENGTH'] !== null)?(int)$column['CHARACTER_MAXIMUM_LENGTH']:null;
	...
	protected function findTableNames($schema = '', $includeViews = true){
		...
		if($includeViews) $condition = "TABLE_TYPE in ('BASE TABLE','VIEW')";
		else $condition = "TABLE_TYPE='BASE TABLE'";
		$sql = <<<EOD
...
EOD;
		...
		foreach($rows as $row){
			...
			else $names[] = $schema . '.' . $row['TABLE_NAME'];
		}
