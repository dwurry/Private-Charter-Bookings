...
class COciSchema extends CDbSchema{
	 ...
	 * @since 1.1.6
	 */
	public $columnTypes = array('pk'=>'NUMBER(10) NOT NULL PRIMARY KEY','string'=>'VARCHAR2(255)','text'=>'CLOB','integer'=>'NUMBER(10)','float'=>'NUMBER','decimal'=>'NUMBER','datetime'=>'TIMESTAMP','timestamp'=>'TIMESTAMP','time'=>'TIMESTAMP','date'=>'DATE','binary'=>'BLOB','boolean'=>'NUMBER(1)','money'=>'NUMBER(19,4)');
	...
	protected function getSchemaTableName($table){
		...
		if(count($parts = explode('.', str_replace('"', '', $table))) > 1) return array($parts[0],$parts[1]);
		else return array($this->getDefaultSchema(),$parts[0]);
	}
	...
	protected function resolveTableNames($table, $name){
		...
		if($schemaName === $this->getDefaultSchema()) $table->rawName = $this->quoteTableName($tableName);
...
EOD;
		...
		$names = array();
		foreach($rows as $row){
			if($schema === $this->getDefaultSchema() || $schema === '') $names[] = $row['TABLE_NAME'];
	...
	public function checkIntegrity($check = true, $schema = ''){
		if($schema === '') $schema = $this->getDefaultSchema();
		$mode = $check?'ENABLE':'DISABLE';
		foreach($this->getTableNames($schema) as $table){
			$constraints = $this->getDbConnection()->createCommand("SELECT CONSTRAINT_NAME FROM USER_CONSTRAINTS WHERE TABLE_NAME=:t AND OWNER=:o")->queryColumn(array(':t'=>$table,':o'=>$schema));
