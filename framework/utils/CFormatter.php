...
class CFormatter extends CApplicationComponent{
	 ...
	 * @since 1.1.11
	 */
	public $sizeFormat = array('base'=>1024,'decimals'=>2,'decimalSeparator'=>null);
	...
	public function __call($name, $parameters){
		if(method_exists($this, 'format' . $name)) return call_user_func_array(array($this,'format' . $name), $parameters);
		else return parent::__call($name, $parameters);
	}
	...
	public function format($value, $type){
		...
		else throw new CException(Yii::t('yii', 'Unknown type "{type}".', array('{type}'=>$type)));
	}
	...
	public function formatNtext($value, $paragraphs = false, $removeEmptyParagraphs = true){
		...
		if($paragraphs){
			...
			if($removeEmptyParagraphs) $value = preg_replace('/(<\/p><p>){2,}/i', '</p><p>', $value);
			return $value;
		}else{
			return nl2br($value);
		}
	...
	protected function normalizeDateValue($time){
		if(is_string($time)){
			if(ctype_digit($time) || ($time{0} == '-' && ctype_digit(substr($time, 1)))) return (int)$time;
			else return strtotime($time);
		}
		return (int)$time;
	}
	...
	public function getHtmlPurifier(){
		if($this->_htmlPurifier === null) $this->_htmlPurifier = new CHtmlPurifier();
