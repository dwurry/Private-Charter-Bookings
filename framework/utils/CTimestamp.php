...
class CTimestamp{
	...
	public static function isLeapYear($year){
		...
		elseif($year > 1582 && $year % 100 == 0) return false;
		return true;
	}
	...
	public static function formatDate($fmt, $d = false, $is_gmt = false){
		...
		if((abs($d) <= 0x7FFFFFFF)){
			...
			if($d >= 0) return ($is_gmt)?@gmdate($fmt, $d):@date($fmt, $d);
		}
		...
		/*
		 * at this point, we have the following integer vars to manipulate: $year, $month, $day, $hour, $min, $secs
		 */
		for($i = 0; $i < $max; $i++){
			switch($fmt[$i]){
				case 'T':
