...
class CMarkdownParser extends MarkdownExtra_Parser{
	...
	public function _doCodeBlocks_callback($matches){
		...
		if(($codeblock = $this->highlightCodeBlock($codeblock)) !== null) return "\n\n" . $this->hashBlock($codeblock) . "\n\n";
	...
	protected function getHighlightTag($codeblock){
		...
		if(strlen($str) > 2 && $str[0] === '[' && $str[strlen($str) - 1] === ']') return $str;
	}
	...
	protected function createHighLighter($options){
		...
		if($highlighter) $highlighter->setRenderer(new Text_Highlighter_Renderer_Html($this->getHighlightConfig($options)));
		return $highlighter;
	}
	...
	public function getHighlightConfig($options){
		...
		if($this->getInlineOption('showLineNumbers', $options, false)) $config['numbers'] = HL_NUMBERS_LI;
	...
	protected function getInlineOption($name, $str, $defaultValue){
		if(preg_match('/' . $name . '(\s*=\s*(\d+))?/i', $str, $v) && count($v) > 2) return $v[2];
