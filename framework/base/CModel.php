...
abstract class CModel extends CComponent implements IteratorAggregate, ArrayAccess{
	...
	protected function afterConstruct(){
		if($this->hasEventHandler('onAfterConstruct')) $this->onAfterConstruct(new CEvent($this));
	}
	...
	public function createValidators(){
		...
		foreach($this->rules() as $rule){
			...
			else throw new CException(Yii::t('yii', '{class} has an invalid validation rule. The rule must specify attributes to be validated and the validator name.', array('{class}'=>get_class($this))));
		}
		return $validators;
	}
	...
	public function hasErrors($attribute = null){
		if($attribute === null) return $this->_errors !== array();
		else return isset($this->_errors[$attribute]);
	}
	...
	public function getErrors($attribute = null){
		...
		else return isset($this->_errors[$attribute])?$this->_errors[$attribute]:array();
	}
	...
	public function clearErrors($attribute = null){
		...
		else unset($this->_errors[$attribute]);
	}
	...
	public function setAttributes($values, $safeOnly = true){
		...
		foreach($values as $name=>$value){
			if(isset($attributes[$name])) $this->$name = $value;
	...
	public function onUnsafeAttribute($name, $value){
		if(YII_DEBUG) Yii::log(Yii::t('yii', 'Failed to set unsafe attribute "{attribute}" of "{class}".', array('{attribute}'=>$name,'{class}'=>get_class($this))), CLogger::LEVEL_WARNING);
	}
