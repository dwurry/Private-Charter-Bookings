...
class CMap extends CComponent implements IteratorAggregate, ArrayAccess, Countable{
	...
	public function itemAt($key){
		if(isset($this->_d[$key])) return $this->_d[$key];
		else return null;
	}
	...
	public function add($key, $value){
		if(!$this->_r){
			if($key === null) $this->_d[] = $value;
			else $this->_d[$key] = $value;
		}else
	...
	public function copyFrom($data){
		if(is_array($data) || $data instanceof Traversable){
			...
			foreach($data as $key=>$value)
				$this->add($key, $value);
		}elseif($data !== null) throw new CException(Yii::t('yii', 'Map data must be an array or an object implementing Traversable.'));
	}
	...
	public static function mergeArray($a, $b){
		...
		while(!empty($args)){
			...
			foreach($next as $k=>$v){
				if(is_integer($k)) isset($res[$k])?$res[] = $v:$res[$k] = $v;
				elseif(is_array($v) && isset($res[$k]) && is_array($res[$k])) $res[$k] = self::mergeArray($res[$k], $v);
				else $res[$k] = $v;
			}
		}
		return $res;
