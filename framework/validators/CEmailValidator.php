...
class CEmailValidator extends CValidator{
	...
	public function validateValue($value){
		if(is_string($value) && $this->validateIDN) $value = $this->encodeIDN($value);
		...
		if($valid) $domain = rtrim(substr($value, strpos($value, '@') + 1), '>');
		if($valid && $this->checkMX && function_exists('checkdnsrr')) $valid = checkdnsrr($domain, 'MX');
		if($valid && $this->checkPort && function_exists('fsockopen') && function_exists('dns_get_record')) $valid = $this->checkMxPorts($domain);
		return $valid;
	}
...
';
		...
		if($this->allowName) $condition .= " && !value.match({$this->fullPattern})";
...
";
	...
	private function encodeIDN($value){
		if(preg_match_all('/^(.*)@(.*)$/', $value, $matches)){
			if(function_exists('idn_to_ascii')) $value = $matches[1][0] . '@' . idn_to_ascii($matches[2][0]);
			else{
