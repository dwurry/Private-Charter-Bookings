...
class CDbFixtureManager extends CApplicationComponent{
	...
	public function init(){
		parent::init();
		if($this->basePath === null) $this->basePath = Yii::getPathOfAlias('application.tests.fixtures');
		$this->prepare();
	}
	...
	public function getDbConnection(){
		if($this->_db === null){
			...
			if(!$this->_db instanceof CDbConnection) throw new CException(Yii::t('yii', 'CDbTestFixture.connectionID "{id}" is invalid. Please make sure it refers to the ID of a CDbConnection application component.', array('{id}'=>$this->connectionID)));
		}
		return $this->_db;
	}
	...
	public function prepare(){
		...
		if(is_file($initFile)) require ($initFile);
		else{
	...
	public function resetTable($tableName){
		...
		if(is_file($initFile)) require ($initFile);
		else $this->truncateTable($tableName);
	}
	...
	public function loadFixture($tableName){
		...
		foreach(require ($fileName) as $alias=>$row){
			...
			if($table->sequenceName !== null){
				if(is_string($primaryKey) && !isset($row[$primaryKey])) $row[$primaryKey] = $builder->getLastInsertID($table);
	...
	public function getFixtures(){
		if($this->_fixtures === null){
			...
			while($file = readdir($folder)){
				if($file === '.' || $file === '..' || $file === $this->initScript) continue;
				...
				if(substr($file, -4) === '.php' && is_file($path) && substr($file, -$suffixLen) !== $this->initScriptSuffix){
					...
					if($schema->getTable($tableName) !== null) $this->_fixtures[$tableName] = $path;
				}
			}
			closedir($folder);
		}
	...
	public function load($fixtures){
		...
		foreach($fixtures as $fixtureName=>$tableName){
			...
			if(($prefix = $this->getDbConnection()->tablePrefix) !== null) $tableName = preg_replace('/{{(.*?)}}/', $prefix . '\1', $tableName);
			$this->resetTable($tableName);
	...
	public function getRows($name){
		if(isset($this->_rows[$name])) return $this->_rows[$name];
		else return false;
	}
