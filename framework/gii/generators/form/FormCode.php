...
class FormCode extends CCodeModel{
	...
	public function requiredTemplates(){
		return array('form.php','action.php');
	}
...
EOD;
	...
	public function validateModel($attribute, $params){
		...
		elseif(!is_subclass_of($class, 'CModel')) $this->addError('model', "'{$this->model}' must extend from CModel.");
		else $this->_modelClass = $class;
	}
	public function validateViewPath($attribute, $params){
		if($this->hasErrors('viewPath')) return;
		if(Yii::getPathOfAlias($this->viewPath) === false) $this->addError('viewPath', 'View Path must be a valid path alias.');
	}
