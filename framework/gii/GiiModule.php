...
class GiiModule extends CWebModule{
	...
	public function getAssetsUrl(){
		if($this->_assetsUrl === null) $this->_assetsUrl = Yii::app()->getAssetManager()->publish(Yii::getPathOfAlias('gii.assets'));
	...
	protected function findGenerators(){
		...
		for($i = $n - 1; $i >= 0; --$i){
			...
			if($path === false || !is_dir($path)) continue;
			...
			foreach($names as $name){
				if($name[0] !== '.' && is_dir($path . '/' . $name)){
					...
					if(is_file("$path/$name/$className.php")){
						$generators[$name] = array('class'=>"$alias.$name.$className");
					}
					...
					if(isset($generators[$name]) && is_dir("$path/$name/templates")){
						...
						foreach($dirs as $dir){
							if($dir[0] !== '.' && is_dir($templatePath . '/' . $dir)) $generators[$name]['templates'][$dir] = strtr($templatePath . '/' . $dir, array('/'=>DIRECTORY_SEPARATOR,'\\'=>DIRECTORY_SEPARATOR));
						}
					}
				}
			}
