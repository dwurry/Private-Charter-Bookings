...
class CCodeFile extends CComponent{
	...
	public function __construct($path, $content){
		...
		if(is_file($path)) $this->operation = file_get_contents($path) === $content?self::OP_SKIP:self::OP_OVERWRITE;
		elseif($content === null) 		// is dir
		$this->operation = is_dir($path)?self::OP_SKIP:self::OP_NEW;
		else $this->operation = self::OP_NEW;
	}
	...
	public function getRelativePath(){
		if(strpos($this->path, Yii::app()->basePath) === 0) return substr($this->path, strlen(Yii::app()->basePath) + 1);
	...
	public function getType(){
		if(($pos = strrpos($this->path, '.')) !== false) return substr($this->path, $pos + 1);
		else return 'unknown';
	}
}