...
class CCodeGenerator extends CController{
	...
	public function actionCode(){
		...
		if(isset($_GET['id']) && isset($model->files[$_GET['id']])){
			$this->renderPartial('/common/code', array('file'=>$model->files[$_GET['id']]));
		}else
	...
	public function actionDiff(){
		...
		if(isset($_GET['id']) && isset($model->files[$_GET['id']])){
			...
			if(!in_array($file->type, array('php','txt','js','css'))) $diff = false;
			elseif($file->operation === CCodeFile::OP_OVERWRITE) $diff = TextDiff::compare(file_get_contents($file->path), $file->content);
			else $diff = '';
	...
	protected function prepare(){
		if($this->codeModel === null) throw new CException(get_class($this) . '.codeModel property must be specified.');
