...
class CDbLogRoute extends CLogRoute{
	...
	protected function createLogTable($db, $tableName){
		$db->createCommand()->createTable($tableName, array('id'=>'pk','level'=>'varchar(128)','category'=>'varchar(128)','logtime'=>'integer','message'=>'text'));
	}
	...
	protected function getDbConnection(){
		...
		elseif(($id = $this->connectionID) !== null){
			...
			else throw new CException(Yii::t('yii', 'CDbLogRoute.connectionID "{id}" does not point to a valid CDbConnection application component.', array('{id}'=>$id)));
		}else{
	...
	protected function processLogs($logs){
		...
		foreach($logs as $log){
			$command->insert($this->logTableName, array('level'=>$log[1],'category'=>$log[2],'logtime'=>(int)$log[3],'message'=>$log[0]));
		}
	}
}
