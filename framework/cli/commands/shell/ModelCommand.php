...
EOD;
	...
	protected function generateClassName($tableName){
		return str_replace(' ', '', ucwords(trim(strtolower(str_replace(array('-','_'), ' ', preg_replace('/(?<![A-Z])[A-Z]/', ' \0', $tableName))))));
	}
	...
	protected function generateClassNames($schema, $pattern = null){
		...
		foreach($schema->getTableNames() as $name){
			if($pattern === null) $this->_tables[$name] = $this->generateClassName($this->removePrefix($name));
			elseif(preg_match($pattern, $name, $matches)){
				if(count($matches) > 1 && !empty($matches[1])) $className = $this->generateClassName($matches[1]);
				else $className = $this->generateClassName($matches[0]);
	...
	public function run($args){
		...
		if(empty($matches[1]))		// without regular expression
		{
			...
			if(($pos = strrpos($className, '.')) === false) $basePath = Yii::getPathOfAlias('application.models');
			else{
		...
		}else		// with regular expression
		{
			...
			else $basePath = Yii::getPathOfAlias('application.models');
		...
		if(count($this->_classes) > 1){
			...
			echo implode("\n", $entries) . "\n\n";
			if(!$this->confirm("Do you want to generate the above classes?")) return;
		}
		...
		foreach($this->_classes as $tableName=>$className){
			...
			$list['models/' . $className . '.php'] = array('source'=>$templatePath . DIRECTORY_SEPARATOR . 'model.php','target'=>$classFile,'callback'=>array($this,'generateModel'),'params'=>array($className,$tableName));
			if($fixturePath !== false){
				$list['fixtures/' . $tableName . '.php'] = array('source'=>$templatePath . DIRECTORY_SEPARATOR . 'fixture.php','target'=>$fixturePath . DIRECTORY_SEPARATOR . $tableName . '.php','callback'=>array($this,'generateFixture'),'params'=>$this->_schema->getTable($tableName));
			}
			if($unitTestPath !== false){
				$fixtureName = $this->pluralize($className);
				$fixtureName[0] = strtolower($fixtureName);
				$list['unit/' . $className . 'Test.php'] = array('source'=>$templatePath . DIRECTORY_SEPARATOR . 'test.php','target'=>$unitTestPath . DIRECTORY_SEPARATOR . $className . 'Test.php','callback'=>array($this,'generateTest'),'params'=>array($className,$fixtureName));
			}
		}
		...
		foreach($files as $className=>$file){
			if(!class_exists($className, false)) include_once ($file);
		}
...
EOD;
	...
	public function generateModel($source, $params){
		...
		if(($table = $this->_schema->getTable($tableName)) !== null){
			...
			foreach($table->columns as $column){
				...
				if($column->isPrimaryKey && $table->sequenceName !== null) continue;
			...
			if(isset($this->_relations[$className]) && is_array($this->_relations[$className])) $relations = $this->_relations[$className];
		}else
		...
		return $this->renderFile($source, array('className'=>$className,'tableName'=>$this->removePrefix($tableName, true),'columns'=>isset($table)?$table->columns:array(),'rules'=>$rules,'labels'=>$labels,'relations'=>$relations), true);
	}
	public function generateFixture($source, $table){
		if(!is_file($source)) 		// fall back to default ones
		$source = YII_PATH . '/cli/views/shell/model/' . basename($source);
		return $this->renderFile($source, array('table'=>$table), true);
	}
	public function generateTest($source, $params){
		list($className, $fixtureName) = $params;
		if(!is_file($source)) 		// fall back to default ones
		$source = YII_PATH . '/cli/views/shell/model/' . basename($source);
		return $this->renderFile($source, array('className'=>$className,'fixtureName'=>$fixtureName), true);
	}
	protected function removePrefix($tableName, $addBrackets = false){
		...
		if($tablePrefix != '' && !strncmp($tableName, $tablePrefix, strlen($tablePrefix))){
			...
			if($addBrackets) $tableName = '{{' . $tableName . '}}';
		}
		return $tableName;
	}
	protected function addPrefix($tableName){
		$tablePrefix = Yii::app()->getDb()->tablePrefix;
		if($tablePrefix != '' && strncmp($tableName, $tablePrefix, strlen($tablePrefix))) $tableName = $tablePrefix . $tableName;
		return $tableName;
	}
}