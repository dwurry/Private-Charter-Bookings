...
EOD;
	...
	public function run($args){
		...
		$module = Yii::app();
		$modelClass = $args[0];
		if(($pos = strpos($modelClass, '.')) === false) $modelClass = 'application.models.' . $modelClass;
		else{
			$id = substr($modelClass, 0, $pos);
			if(($m = Yii::app()->getModule($id)) !== null) $module = $m;
		}
		...
		if(isset($args[1])){
			...
			if(($pos = strrpos($controllerID, '/')) === false){
				...
				if(($m = Yii::app()->getModule($first)) !== null) $module = $m;
				else{
		...
		$list = array(basename($controllerFile)=>array('source'=>$templatePath . '/controller.php','target'=>$controllerFile,'callback'=>array($this,'generateController'),'params'=>array($controllerClass,$modelClass)));
		
		if($functionalTestPath !== false){
			$list[$modelClass . 'Test.php'] = array('source'=>$templatePath . '/test.php','target'=>$functionalTestPath . DIRECTORY_SEPARATOR . $modelClass . 'Test.php','callback'=>array($this,'generateTest'),'params'=>array($controllerID,$fixtureName,$modelClass));
		}
		
		foreach($this->actions as $action){
			$list[$action . '.php'] = array('source'=>$templatePath . '/' . $action . '.php','target'=>$viewPath . '/' . $action . '.php','callback'=>array($this,'generateView'),'params'=>$modelClass);
		}
		
		$this->copyFiles($list);
		
		if($module instanceof CWebModule) $moduleID = $module->id . '/';
		else $moduleID = '';
	...
	public function generateController($source, $params){
		...
		if($id === null) throw new ShellException(Yii::t('yii', 'Error: Table "{table}" does not have a primary key.', array('{table}'=>$model->tableName())));
		elseif(is_array($id)) throw new ShellException(Yii::t('yii', 'Error: Table "{table}" has a composite primary key which is not supported by crud command.', array('{table}'=>$model->tableName())));
		
		if(!is_file($source)) 		// fall back to default ones
		$source = YII_PATH . '/cli/views/shell/crud/' . basename($source);
		
		return $this->renderFile($source, array('ID'=>$id,'controllerClass'=>$controllerClass,'modelClass'=>$modelClass), true);
	}
	public function generateView($source, $modelClass){
		...
		if(!is_file($source)) 		// fall back to default ones
		$source = YII_PATH . '/cli/views/shell/crud/' . basename($source);
		return $this->renderFile($source, array('ID'=>$table->primaryKey,'modelClass'=>$modelClass,'columns'=>$columns), true);
	}
	public function generateTest($source, $params){
		list($controllerID, $fixtureName, $modelClass) = $params;
		if(!is_file($source)) 		// fall back to default ones
		$source = YII_PATH . '/cli/views/shell/crud/' . basename($source);
		return $this->renderFile($source, array('controllerID'=>$controllerID,'fixtureName'=>$fixtureName,'modelClass'=>$modelClass), true);
	}
	...
	public function class2name($className, $pluralize = false){
		if($pluralize) $className = $this->pluralize($className);
