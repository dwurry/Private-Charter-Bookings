...
class CConsoleApplication extends CApplication{
	...
	public function processRequest(){
		...
		if(is_int($exitCode)) $this->end($exitCode);
	}
	...
	public function displayError($code, $message, $file, $line){
		...
		if(count($trace) > 4) $trace = array_slice($trace, 4);
	...
	public function getCommandPath(){
		...
		if($this->_commandPath === null && file_exists($applicationCommandPath)) $this->setCommandPath($applicationCommandPath);
	...
	public function setCommandPath($value){
		if(($this->_commandPath = realpath($value)) === false || !is_dir($this->_commandPath)) throw new CException(Yii::t('yii', 'The command path "{path}" is not a valid directory.', array('{path}'=>$value)));
	}
