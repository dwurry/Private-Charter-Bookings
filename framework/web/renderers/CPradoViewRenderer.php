...
class CPradoViewRenderer extends CViewRenderer{
	...
	protected function generateViewFile($sourceFile, $viewFile){
		static $regexRules = array('<%=?\s*(.*?)\s*%>',		// PHP statements or expressions
		...
		for($i = 0; $i < $n; ++$i){
			...
			if($matchStart > $textStart) $this->_output .= substr($this->_input, $textStart, $matchStart - $textStart);
			...
			if(strpos($str, '<com:') === 0)			// opening component tag
			{
				...
				else $this->_output .= $this->processWidget($type, $match[4][0], $match[2][1]);
			...
			elseif(strpos($str, '<cache:') === 0)			// opening cache tag
			{
				...
				else $this->_output .= $this->processCache($id, $match[4][0], $match[2][1]);
			...
			elseif(strpos($str, '<clip:') === 0)			// opening clip tag
			{
				...
				else $this->_output .= $this->processClip($id, $match[4][0], $match[2][1]);
		...
		if($textStart < strlen($this->_input)) $this->_output .= substr($this->_input, $textStart);
		
		file_put_contents($viewFile, $this->_output);
	}
	
	/*
	 * @param string $type type @param string $attributes attributes @param string $offset offset
	 */
	private function processWidget($type, $attributes, $offset){
		$attrs = $this->processAttributes($attributes);
		if(empty($attrs)) return $this->generatePhpCode("\$this->widget('$type');", $offset);
		else return $this->generatePhpCode("\$this->widget('$type', array($attrs));", $offset);
	}
	...
	private function processBeginWidget($type, $attributes, $offset){
		$attrs = $this->processAttributes($attributes);
		if(empty($attrs)) return $this->generatePhpCode("\$this->beginWidget('$type');", $offset);
		else return $this->generatePhpCode("\$this->beginWidget('$type', array($attrs));", $offset);
	}
	...
	/*
	 * @param string $id id @param string $attributes attributes @param string $offset offset
	 */
	...
	private function processBeginCache($id, $attributes, $offset){
		$attrs = $this->processAttributes($attributes);
		if(empty($attrs)) return $this->generatePhpCode("if(\$this->beginCache('$id')):", $offset);
		else return $this->generatePhpCode("if(\$this->beginCache('$id', array($attrs))):", $offset);
	}
	...
	private function processBeginClip($id, $attributes, $offset){
		$attrs = $this->processAttributes($attributes);
		if(empty($attrs)) return $this->generatePhpCode("\$this->beginClip('$id');", $offset);
		else return $this->generatePhpCode("\$this->beginClip('$id', array($attrs));", $offset);
	}
	...
	private function processAttributes($str){
		...
		for($i = 0; $i < $n; ++$i){
			...
			$name = $match[1];
			$value = $match[2];
			if($value[0] === '{') $attributes[] = "'$name'=>" . str_replace('__FILE__', $this->_sourceFile, substr($value, 1, -1));
