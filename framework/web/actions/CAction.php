...
abstract class CAction extends CComponent implements IAction{
	...
	public function runWithParams($params){
		...
		if($method->getNumberOfParameters() > 0) return $this->runWithParamsInternal($this, $method, $params);
	...
	protected function runWithParamsInternal($object, $method, $params){
		...
		foreach($method->getParameters() as $i=>$param){
			...
			if(isset($params[$name])){
				if($param->isArray()) $ps[] = is_array($params[$name])?$params[$name]:array($params[$name]);
				elseif(!is_array($params[$name])) $ps[] = $params[$name];
				else return false;
			}elseif($param->isDefaultValueAvailable()) $ps[] = $param->getDefaultValue();
			else return false;
		}
