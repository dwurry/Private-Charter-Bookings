...
class CWidget extends CBaseController{
	...
	public function getId($autoGenerate = true){
		...
		elseif($autoGenerate) return $this->_id = 'yw' . self::$_counter++;
	}
	...
	public function getController(){
		if($this->_owner instanceof CController) return $this->_owner;
		else return Yii::app()->getController();
	}
	...
	public function getViewPath($checkTheme = false){
		...
		if(isset(self::$_viewPaths[$className][$scope])) return self::$_viewPaths[$className][$scope];
		else{
			if($checkTheme && ($theme = Yii::app()->getTheme()) !== null){
				...
				if(is_dir($path)) return self::$_viewPaths[$className]['theme'] = $path;
			}
	...
	public function render($view, $data = null, $return = false){
		if(($viewFile = $this->getViewFile($view)) !== false) return $this->renderFile($viewFile, $data, $return);
		else throw new CException(Yii::t('yii', '{widget} cannot find the view "{view}".', array('{widget}'=>get_class($this),'{view}'=>$view)));
	}
}