...
class CMultiFileUpload extends CInputWidget{
	...
	protected function getClientOptions(){
		...
		foreach(array('onFileRemove','afterFileRemove','onFileAppend','afterFileAppend','onFileSelect','afterFileSelect') as $event){
			if(isset($options[$event]) && !($options[$event] instanceof CJavaScriptExpression)) $options[$event] = new CJavaScriptExpression($options[$event]);
		}
		
		if($this->accept !== null) $options['accept'] = $this->accept;
		...
		if($messages !== array()) $options['STRING'] = $messages;
