...
class CWebUser extends CApplicationComponent implements IWebUser{
	...
	public function __isset($name){
		if($this->hasState($name)) return $this->getState($name) !== null;
	...
	public function __unset($name){
		if($this->hasState($name)) $this->setState($name, null);
	...
	public function init(){
		...
		if($this->getIsGuest() && $this->allowAutoLogin) $this->restoreFromCookie();
		elseif($this->autoRenewCookie && $this->allowAutoLogin) $this->renewCookie();
	...
	public function login($identity, $duration = 0){
		...
		if($this->beforeLogin($id, $states, false)){
			...
			if($duration > 0){
				...
				else throw new CException(Yii::t('yii', '{class}.allowAutoLogin must be set true in order to use cookie-based authentication.', array('{class}'=>get_class($this))));
			}
			
			if($this->absoluteAuthTimeout) $this->setState(self::AUTH_ABSOLUTE_TIMEOUT_VAR, time() + $this->absoluteAuthTimeout);
	...
	public function logout($destroySession = true){
		if($this->beforeLogout()){
			...
			if($destroySession) Yii::app()->getSession()->destroy();
	...
	public function getName(){
		if(($name = $this->getState('__name')) !== null) return $name;
	...
	protected function saveToCookie($duration){
		...
		$data = array($this->getId(),$this->getName(),$duration,$this->saveIdentityStates());
	...
	public function setState($key, $value, $defaultValue = null){
		$key = $this->getStateKeyPrefix() . $key;
		if($value === $defaultValue) unset($_SESSION[$key]);
	...
	public function getFlashes($delete = true){
		...
		if($delete) $this->setState(self::FLASH_COUNTERS, array());
		return $flashes;
	}
	...
	public function setFlash($key, $value, $defaultValue = null){
		...
		if($value === $defaultValue) unset($counters[$key]);
		else $counters[$key] = 0;
	...
	protected function updateAuthStatus(){
		if(($this->authTimeout !== null || $this->absoluteAuthTimeout !== null) && !$this->getIsGuest()){
			...
			if($expires !== null && $expires < time() || $expiresAbsolute !== null && $expiresAbsolute < time()) $this->logout(false);
			else $this->setState(self::AUTH_TIMEOUT_VAR, time() + $this->authTimeout);
		}
	}
	...
	public function checkAccess($operation, $params = array(), $allowCaching = true){
		...
		if($allowCaching && $params === array()) $this->_access[$operation] = $access;
