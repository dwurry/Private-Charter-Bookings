...
class CSqlDataProvider extends CDataProvider{
	...
	protected function fetchData(){
		...
		if(($sort = $this->getSort()) !== false){
			...
			if(!empty($order)){
				if(preg_match('/\s+order\s+by\s+[\w\s,\.]+$/i', $command->text)) $command->text .= ', ' . $order;
				else $command->text .= ' ORDER BY ' . $order;
			}
		}
	...
	protected function fetchKeys(){
		...
		if($data = $this->getData()){
			if(is_object(reset($data))) foreach($data as $i=>$item)
				$keys[$i] = $item->{$this->keyField};
			else foreach($data as $i=>$item)
				$keys[$i] = $item[$this->keyField];
		}
		return $keys;
	}
