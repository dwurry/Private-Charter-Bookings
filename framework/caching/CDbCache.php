...
class CDbCache extends CCache{
	...
	protected function createCacheTable($db, $tableName){
		...
		if($driver === 'mysql') $blob = 'LONGBLOB';
		elseif($driver === 'pgsql') $blob = 'BYTEA';
		else $blob = 'BLOB';
		$sql = <<<EOD
...
EOD;
	...
	public function getDbConnection(){
		...
		elseif(($id = $this->connectionID) !== null){
			...
			else throw new CException(Yii::t('yii', 'CDbCache.connectionID "{id}" is invalid. Please make sure it refers to the ID of a CDbConnection application component.', array('{id}'=>$id)));
		}else{
