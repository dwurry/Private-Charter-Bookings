...
class CFileCache extends CCache{
	...
	protected function getValue($key){
		...
		if(($time = $this->filemtime($cacheFile)) > time()) return @file_get_contents($cacheFile, false, null, $this->embedExpiry?10:-1);
		elseif($time > 0) @unlink($cacheFile);
		return false;
	}
	...
	protected function setValue($key, $value, $expire){
		...
		$cacheFile = $this->getCacheFile($key);
		if($this->directoryLevel > 0) @mkdir(dirname($cacheFile), 0777, true);
	...
	protected function addValue($key, $value, $expire){
		$cacheFile = $this->getCacheFile($key);
		if($this->filemtime($cacheFile) > time()) return false;
	...
	protected function getCacheFile($key){
		if($this->directoryLevel > 0){
			...
			for($i = 0; $i < $this->directoryLevel; ++$i){
				if(($prefix = substr($key, $i + $i, 2)) !== false) $base .= DIRECTORY_SEPARATOR . $prefix;
			}
	...
	public function gc($expiredOnly = true, $path = null){
		...
		while(($file = readdir($handle)) !== false){
			...
			if(is_dir($fullPath)) $this->gc($expiredOnly, $fullPath);
			elseif($expiredOnly && $this->filemtime($fullPath) < time() || !$expiredOnly) @unlink($fullPath);
		}
		closedir($handle);
	}
	...
	private function filemtime($path){
		if($this->embedExpiry) return (int)@file_get_contents($path, false, null, 0, 10);
		else return @filemtime($path);
	}
}
