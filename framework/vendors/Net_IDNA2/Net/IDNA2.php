...
class Net_IDNA2{
		...
		foreach($decoded as $k=>$v){
			...
			switch($v){
				...
				case 0x40:
					...
					if($this->_strict_mode){throw new InvalidArgumentException('Neither email addresses nor URLs are allowed in strict mode.');}
					// Skip first char
					if($k){
						$encoded = '';
	...
	public function decode($input, $one_time_encoding = false){
		...
		if(strpos($input, '@')){ // Maybe it is an email address
			...
			if($this->_strict_mode){throw new InvalidArgumentException('Only simple domain name parts can be handled in strict mode');}
	...
	private function _encode($decoded){
		...
		if($check_pref == $check_deco){throw new InvalidArgumentException('This is already a punycode string');}
		...
		if(!$encodable){
			if($this->_strict_mode){throw new InvalidArgumentException('The given string does not contain encodable chars');}
	...
	private function _decode($encoded){
		...
		if(!preg_match('!^' . preg_quote($this->_punycode_prefix, '!') . '!', $encoded)){return false;}
		
		$encode_test = preg_replace('!^' . preg_quote($this->_punycode_prefix, '!') . '!', '', $encoded);
		
		// If nothing left after removing the prefix, it is hopeless
		if(!$encode_test){return false;}
	...
	private function _nameprep($input){
		...
		foreach($input as $v){
			...
			if(in_array($v, self::$_np_prohibit) || in_array($v, self::$_general_prohibited)){throw new Net_IDNA2_Exception_Nameprep('Prohibited input U+' . sprintf('%08X', $v));}
			
			foreach(self::$_np_prohibit_ranges as $range){
				if($range[0] <= $v && $v <= $range[1]){throw new Net_IDNA2_Exception_Nameprep('Prohibited input U+' . sprintf('%08X', $v));}
			}
	...
	private function _utf8_to_ucs4($input){
		...
		for($k = 0; $k < $inp_len; ++$k){
			...
			if($v < 128){ // We found an ASCII char - put into stirng as is
				$output[$out_len] = $v;
				++$out_len;
				if('add' == $mode){throw new UnexpectedValueException('Conversion from UTF-8 to UCS-4 failed: malformed input at byte ' . $k);}
				continue;
			}
			...
			if('add' == $mode){
				if(!$this->_allow_overlong && $test == 'range'){
					$test = 'none';
					if(($v < 0xA0 && $start_byte == 0xE0) || ($v < 0x90 && $start_byte == 0xF0) || ($v > 0x8F && $start_byte == 0xF4)){throw new OutOfRangeException('Bogus UTF-8 character detected (out of legal range) at byte ' . $k);}
				}
	...
	private function _ucs4_string_to_ucs4($input){
		...
		if($inp_len % 4){throw new InvalidArgumentException('Input UCS4 string is broken');}
	...
	private static function _byteLength($string){
		if(self::$_mb_string_overload){return mb_strlen($string, '8bit');}
