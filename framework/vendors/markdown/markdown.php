...
class Markdown_Parser{
	...
	public $em_relist = array(''=>'(?:(?<!\*)\*(?!\*)|(?<!_)_(?!_))(?=\S|$)(?![\.,:;]\s)','*'=>'(?<=\S|^)(?<!\*)\*(?!\*)','_'=>'(?<=\S|^)(?<!_)_(?!_)');
	public $strong_relist = array(''=>'(?:(?<!\*)\*\*(?!\*)|(?<!_)__(?!_))(?=\S|$)(?![\.,:;]\s)','**'=>'(?<=\S|^)(?<!\*)\*\*(?!\*)','__'=>'(?<=\S|^)(?<!_)__(?!_)');
	public $em_strong_relist = array(''=>'(?:(?<!\*)\*\*\*(?!\*)|(?<!_)___(?!_))(?=\S|$)(?![\.,:;]\s)','***'=>'(?<=\S|^)(?<!\*)\*\*\*(?!\*)','___'=>'(?<=\S|^)(?<!_)___(?!_)');
...
class MarkdownExtra_Parser extends Markdown_Parser{
	...
	public function setup(){
		...
		foreach($this->predef_abbr as $abbr_word=>$abbr_desc){
			if($this->abbr_word_re) $this->abbr_word_re .= '|';
...
preg_match ( '{^</?(?:' . $enclosing_tag_re . ')\b}', $tag )) 
	...
	public function _hashHTMLBlocks_inHTML($text, $hash_method, $md_attr){
		...
		//
		              ...
		              //
		if(preg_match('/^<([\w:$]*)\b/', $text, $matches)) $base_tag_name_re = $matches[1];
	...
	//
	public $em_relist = array(''=>'(?:(?<!\*)\*(?!\*)|(?<![a-zA-Z0-9_])_(?!_))(?=\S|$)(?![\.,:;]\s)','*'=>'(?<=\S|^)(?<!\*)\*(?!\*)','_'=>'(?<=\S|^)(?<!_)_(?![a-zA-Z0-9_])');
	public $strong_relist = array(''=>'(?:(?<!\*)\*\*(?!\*)|(?<![a-zA-Z0-9_])__(?!_))(?=\S|$)(?![\.,:;]\s)','**'=>'(?<=\S|^)(?<!\*)\*\*(?!\*)','__'=>'(?<=\S|^)(?<!_)__(?![a-zA-Z0-9_])');
	public $em_strong_relist = array(''=>'(?:(?<!\*)\*\*\*(?!\*)|(?<![a-zA-Z0-9_])___(?!_))(?=\S|$)(?![\.,:;]\s)','***'=>'(?<=\S|^)(?<!\*)\*\*\*(?!\*)','___'=>'(?<=\S|^)(?<!_)___(?![a-zA-Z0-9_])');
	public function formParagraphs($text){
		//
		// Params:
		// $text - string to process with html <p> tags
