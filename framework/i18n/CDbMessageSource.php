...
class CDbMessageSource extends CMessageSource{
	...
	protected function loadMessages($category, $language){
		...
		if(isset($cache)) $cache->set($key, serialize($messages), $this->cachingDuration);
	...
	public function getDbConnection(){
		if($this->_db === null){
			...
			if(!$this->_db instanceof CDbConnection) throw new CException(Yii::t('yii', 'CDbMessageSource.connectionID is invalid. Please make sure "{id}" refers to a valid database application component.', array('{id}'=>$this->connectionID)));
		}
		return $this->_db;
	}
