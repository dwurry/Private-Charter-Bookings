...
class BootGroupGridView extends TbGridView{
	...
	public function groupByColumns(){
		...
		foreach($values as $colName=>$value){
			$groups[$colName][] = array('value'=>$value,'column'=>$colName,'start'=>0 
			// end - later
			// totals - later
						))
;
		...
		for($i = 1; $i < count($data); $i++){
			...
			/*
			 * if this flag = true -> we will write change for all grouping columns. It's required when change of any column from extraRowColumns occurs
			 */
			$extraRowColumnChanged = (count(array_intersect($changedColumns, $this->extraRowColumns)) > 0);
			
			/*
			 * this changeOccured related to foreach below. It is required only for mergeType == self::MERGE_NESTED, to write change for all nested columns when change of previous column occured
			 */
			$changeOccured = false;
			foreach($current as $colName=>$curValue){
				// value changed
				...
				if($extraRowColumnChanged || $saveChange){
					...
					// begin new group
					$groups[$colName][] = array('start'=>$i,'column'=>$colName,'value'=>$curValue);
				}
			}
	...
	public function renderTableRow($row){
		...
		/*
		 * if($this->_changes && array_key_exists($row, $this->_changes)) { $change = $this->_changes[$row]; //if change in extracolumns --> put extra row $columnsInExtra = array_intersect(array_keys($change['columns']), $this->extraRowColumns); //extraRowPos = before if(count($columnsInExtra) > 0 && $this->extraRowPos == 'before') { $this->renderExtraRow($row, $this->_changes[$row], $columnsInExtra); } }
		 */
		...
		}else if(is_array($this->rowCssClass) && ($n = count($this->rowCssClass)) > 0) echo '<tr class="' . $this->rowCssClass[$row % $n] . '">';
		else echo '<tr>';
	...
	private function getDataCellContent($column, $data, $row){
		if($column->value !== null) $value = $column->evaluateExpression($column->value, array('data'=>$data,'row'=>$row));
		else if($column->name !== null) $value = CHtml::value($data, $column->name);
