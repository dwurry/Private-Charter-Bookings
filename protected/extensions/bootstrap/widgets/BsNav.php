...
class BsNav extends CWidget{
	...
	protected function isItemActive($item, $route){
		if(!isset($item['url']) || (is_string($item['url']) && trim($item['url']) === '')) return false;
		
		$itemUrl = trim($item['url'][0], '/');
		
		if(substr_count($itemUrl, '/') === 0 && isset(Yii::app()->controller->module->id) && isset(Yii::app()->controller->id)) $itemUrl = Yii::app()->controller->module->id . '/' . Yii::app()->controller->id . '/' . $itemUrl;
		elseif(substr_count($itemUrl, '/') === 0 && isset(Yii::app()->controller->id)) $itemUrl = Yii::app()->controller->id . '/' . $itemUrl;
		elseif(substr_count($itemUrl, '/') === 1 && isset(Yii::app()->controller->module->id)) $itemUrl = Yii::app()->controller->module->id . '/' . $itemUrl;
