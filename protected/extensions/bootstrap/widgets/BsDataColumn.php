 ...
 */
class BsDataColumn extends CDataColumn{
	/**
	 *
	...
	protected function renderHeaderCellContent(){
		if($this->grid->enableSorting && $this->sortable && $this->name !== null){
			...
			if(isset($sort->directions[$this->name])){
				if($sort->directions[$this->name]) $label .= ' <span class="dropup"><span class="caret"></span></span>';
				else $label .= ' <span class="caret"></span>';
			}
	 ...
	 * Here we can provide HTML options for actual filter input
	 */
	protected function renderFilterCellContent(){
		if(is_string($this->filter)){
			...
			if($this->filter !== false && $this->grid->filter !== null && $this->name !== null && strpos($this->name, '.') === false){
				...
				if(isset($filterInputOptions['class'])) $filterInputOptions['class'] .= ' form-control';
				else $filterInputOptions['class'] = 'form-control';
