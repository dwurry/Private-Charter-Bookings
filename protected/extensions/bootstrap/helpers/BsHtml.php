...
class BsHtml extends CHtml{
	 ...
	 */
	public static function statefulFormBs($layout = self::FORM_LAYOUT_VERTICAL, $action = '', $method = 'post', $htmlOptions = array()){
		return self::formBs($layout, $action, $method, $htmlOptions) . self::tag('div', array('style'=>'display: none'), parent::pageStateField(''));
	}
	 ...
	 */
	public static function formBs($layout = self::FORM_LAYOUT_VERTICAL, $action = '', $method = 'post', $htmlOptions = array()){
	 ...
	 * @return string the generated tag.
	 */
	public static function beginFormBs($layout = self::FORM_LAYOUT_VERTICAL, $action = '', $method = 'post', $htmlOptions = array()){
		if(!empty($layout)){
	...
	public static function checkBoxList($name, $select, $data, $htmlOptions = array()){
		...
		if($inline){
			...
			$template = BsArray::popValue('template', $htmlOptions, parent::tag('div', array('class'=>'checkbox'), '{beginLabel}{input}{labelTitle}{endLabel}'));
		}
...
EOD;
	...
	public static function radioButtonList($name, $select, $data, $htmlOptions = array()){
		...
		if($inline){
			...
			$template = BsArray::popValue('template', $htmlOptions, parent::tag('div', array('class'=>'radio'), '{beginLabel}{input}{labelTitle}{endLabel}'));
		}
	...
	public static function checkBoxControlGroup($name, $checked = false, $htmlOptions = array()){
		...
		if($color){
			if($layout === BsHtml::FORM_LAYOUT_HORIZONTAL) self::addCssClass($color, $groupOptions);
			else self::addCssClass($color, $controlOptions);
		}
		...
		if(!empty($help)) $labelContent .= self::inputHelp($help, $helpOptions);
	...
	public static function activeTextArea($model, $attribute, $htmlOptions = array()){
		$htmlOptions = self::normalizeInputOptions($htmlOptions);
		if(!isset($htmlOptions['class'])) $htmlOptions['class'] = 'form-control';
		
		if(!isset($htmlOptions['rows'])) $htmlOptions['rows'] = '5';
	...
	public static function activeCheckBoxControlGroup($model, $attribute, $htmlOptions = array()){
		...
		if(!empty($help) && !$error) $help = self::inputHelp($help, $helpOptions);
		...
		$label = self::label($labelTitle, $htmlOptions['id'], $labelOptions);
		$endLabel = self::closeTag('label');
		$output .= strtr($template, array('{input}'=>$input,'{beginLabel}'=>$beginLabel,'{label}'=>$label,'{labelTitle}'=>$labelTitle,'{endLabel}'=>$endLabel,'{error}'=>$error,'{help}'=>$help));
	...
	public static function buttonGroup(array $buttons, $htmlOptions = array()){
		if(!empty($buttons)){
			...
			$parentOptions = array('color'=>BsHtml::BUTTON_COLOR_DEFAULT,'size'=>BsArray::popValue('size', $htmlOptions),'disabled'=>BsArray::popValue('disabled', $htmlOptions));
			...
			foreach($buttons as $buttonOptions){
				...
				if(!empty($items)){
					...
					if(!empty($type) && $type = self::BUTTON_TYPE_LINK) $output .= self::linkButton($buttonLabel, $buttonOptions);
					else $output .= self::button($buttonLabel, $buttonOptions);
				}
			}
			$output .= '</div>';
			return $output;
	...
	public static function buttonToolbar(array $groups, $htmlOptions = array()){
		if(!empty($groups)){
			...
			$parentOptions = array('color'=>BsArray::popValue('color', $htmlOptions),'size'=>BsArray::popValue('size', $htmlOptions),'disabled'=>BsArray::popValue('disabled', $htmlOptions));
			$output = self::openTag('div', $htmlOptions);
	...
	public static function nav($type, $items, $htmlOptions = array()){
		...
		if(!empty($type)){
			if($type == 'navbar') self::addCssClass('navbar-nav', $htmlOptions);
			else self::addCssClass('nav-' . $type, $htmlOptions);
		}
	...
	public static function menuText($label, $htmlOptions = array()){
		...
		if($pull) self::addCssClass('pull-' . $pull, $htmlOptions);
	...
	public static function rowThumbnails(array $thumbnails, $htmlOptions){
		if(empty($thumbnails)) return false;
		
		$output = self::openTag('div', array('class'=>'row'));
		$row = BsArray::popValue('row', $htmlOptions, false);
		if(!$row){
			$row = array('lg'=>'3','md'=>'4','sm'=>'6','xs'=>'12');
		}
		foreach($thumbnails as $timage){
			$output .= self::openTag('div', array('class'=>self::generateThumbnailRowClass($row)));
	...
	protected static function bar($width = 0, $htmlOptions = array()){
		...
		$default = array('role'=>'progressbar',"aria-valuenow"=>$width,"aria-valuemin"=>0,"aria-valuemax"=>100);
	...
	public static function bsLabel($label, $htmlOptions = array()){
		...
		$color = BsArray::popValue('color', $htmlOptions);
		if(!empty($color)) self::addCssClass('label label-' . $color, $htmlOptions);
		else self::addCssClass('label label-default', $htmlOptions);
