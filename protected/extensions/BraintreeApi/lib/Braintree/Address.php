...
class Braintree_Address extends Braintree{
	...
	public static function create($attribs){
		...
		return self::_doCreate('/customers/' . $customerId . '/addresses', array('address'=>$attribs));
	}
	...
	public static function createSignature(){
		return array('company','countryCodeAlpha2','countryCodeAlpha3','countryCodeNumeric','countryName','customerId','extendedAddress','firstName','lastName','locality','postalCode','region','streetAddress');
	}
	...
	private static function _validateId($id = null){
		if(empty($id) || trim($id) == ""){throw new InvalidArgumentException('expected address id to be set');}
		if(!preg_match('/^[0-9A-Za-z_-]+$/', $id)){throw new InvalidArgumentException($id . ' is an invalid address id.');}
	}
	...
	private static function _validateCustomerId($id = null){
		if(empty($id) || trim($id) == ""){throw new InvalidArgumentException('expected customer id to be set');}
