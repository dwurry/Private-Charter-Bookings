...
class Braintree_Configuration extends Braintree{
	 ...
	 *
	 */
	private static $_cache = array('environment'=>'sandbox','merchantId'=>'rhbp25kgxkcnhpmm','publicKey'=>'24x38b2zbs9ys4y9','privateKey'=>'c87894424439f0e9cfd9af7ebe79e123','clientsideKey'=>'');
	/**
	 *
	 * @access protected
	 * @static
	 ...
	 */
	private static $_validEnvironments = array('development','sandbox','production','qa');
	...
	private static function validate($key = null, $value = null){
		if(empty($key) && empty($value)){throw new InvalidArgumentException('nothing to validate');}
		
		if($key === 'environment' && !in_array($value, self::$_validEnvironments)){throw new Braintree_Exception_Configuration('"' . $value . '" is not a valid environment.');}
		
		if(!isset(self::$_cache[$key])){throw new Braintree_Exception_Configuration($key . ' is not a valid configuration setting.');}
		
		if(empty($value)){throw new InvalidArgumentException($key . ' cannot be empty.');}
	...
	private static function get($key){
		...
		if(isset(self::$_cache[$key]) && (empty(self::$_cache[$key]))){throw new Braintree_Exception_Configuration($key . ' needs to be set');}
	 ...
	 *
	 * @param string $value
	 *        pass a string to set, empty to get
	...
	public static function clientsideKey($value = null){
		return self::setOrGet(__FUNCTION__, $value);
	}
	/**
	 * #@-
	...
	public static function caFile($sslPath = NULL){
		...
		if(!file_exists($caPath)){throw new Braintree_Exception_SSLCaFileNotFound();}
