...
class Braintree_CreditCard extends Braintree{
	...
	public static function createFromTransparentRedirect($queryString){
		...
		return self::_doCreate('/payment_methods/all/confirm_transparent_redirect_request', array('id'=>$params['id']));
	}
	...
	public static function expired(){
		...
		$pager = array('className'=>__CLASS__,'classMethod'=>'fetchExpired','methodArgs'=>array());
	...
	public static function expiringBetween($startDate, $endDate){
		...
		$pager = array('className'=>__CLASS__,'classMethod'=>'fetchExpiring','methodArgs'=>array($startDate,$endDate));
	...
	public static function updateFromTransparentRedirect($queryString){
		trigger_error("DEPRECATED: Please use Braintree_TransparentRedirectRequest::confirm", E_USER_NOTICE);
		$params = Braintree_TransparentRedirect::parseAndValidateQueryString($queryString);
		return self::_doUpdate('post', '/payment_methods/all/confirm_transparent_redirect_request', array('id'=>$params['id']));
	}
	...
	private static function baseSignature($options){
		return array('billingAddressId','cardholderName','cvv','number','expirationDate','expirationMonth','expirationYear','token','venmoSdkPaymentMethodCode',array('options'=>$options),array('billingAddress'=>array('firstName','lastName','company','countryCodeAlpha2','countryCodeAlpha3','countryCodeNumeric','countryName','extendedAddress','locality','region','postalCode','streetAddress')));
	}
	...
	public static function updateSignature(){
		...
		$updateExistingBillingSignature = array(array('options'=>array('updateExisting')));
	...
	private static function _validateId($token = null){
		if(empty($token)){throw new InvalidArgumentException('expected credit card id to be set');}
		if(!preg_match('/^[0-9A-Za-z_-]+$/', $token)){throw new InvalidArgumentException($token . ' is an invalid credit card id.');}
	}
	...
	public static function factory($attributes){
		$defaultAttributes = array('bin'=>'','expirationMonth'=>'','expirationYear'=>'','last4'=>'');
