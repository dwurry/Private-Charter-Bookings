...
class Image{
	 ...
	 * @param
	 *        array non-default configurations
	 * @return object
	 */
	...
	public function __construct($image, $config = NULL){
		...
		if($check === FALSE) throw new CException('image getimagesize missing');
		...
		if(!is_array($image_info) or count($image_info) < 3) throw new CException('image file unreadable');
		
		// Check to make sure the image type is allowed
		if(!isset(Image::$allowed_types[$image_info[2]])) throw new CException('image type not allowed');
		...
		// Load configuration
		if($config === null){
			$this->config = array('driver'=>'GD','params'=>array());
		}else{
			$this->config = $config;
		}
		...
		// Validate the driver
		if(!($this->driver instanceof Image_Driver)) throw new CException('image driver must be implement Image_Driver class');
	}
	 ...
	 *        integer height
	 * @param
	 *        integer one of: Image::NONE, Image::AUTO, Image::WIDTH, Image::HEIGHT
	 * @return object
	 */
	public function resize($width, $height, $master = NULL){
		if(!$this->valid_size('width', $width)) throw new CException('image invalid width');
		
		if(!$this->valid_size('height', $height)) throw new CException('image invalid height');
		
		if(empty($width) and empty($height)) throw new CException('image invalid dimensions');
		
		if($master === NULL){
			// Maintain the aspect ratio by default
			$master = Image::AUTO;
		}elseif(!$this->valid_size('master', $master)) throw new CException('image invalid master');
	 ...
	 *        integer height
	 * @param
	 *        integer top offset, pixel value or one of: top, center, bottom
	 * @param
	 *        integer left offset, pixel value or one of: left, center, right
	 * @return object
	 */
	public function crop($width, $height, $top = 'center', $left = 'center'){
		if(!$this->valid_size('width', $width)) throw new CException('image invalid width', $width);
		
		if(!$this->valid_size('height', $height)) throw new CException('image invalid height', $height);
		
		if(!$this->valid_size('top', $top)) throw new CException('image invalid top', $top);
		
		if(!$this->valid_size('left', $left)) throw new CException('image invalid left', $left);
	...
	public function flip($direction){
		if($direction !== self::HORIZONTAL and $direction !== self::VERTICAL) throw new CException('image invalid flip');
	 ...
	 *
	 * @param
	 *        integer quality as a percentage
	 * @return object
	 */
	 ...
	 *
	 * @param
	 *        integer amount to sharpen, usually ~20 is ideal
	 * @return object
	 */
	 ...
	 * @throws Kohana_Exception
	 * @param
	 *        string new image filename
	 * @param
	 *        integer permissions for new image
	 * @param
	 *        boolean keep or discard image process actions
	 * @return object
	 */
	public function save($new_image = FALSE, $chmod = 0644, $keep_actions = FALSE){
		...
		if(!is_writable($dir)) throw new CException('image directory unwritable');
		...
		if($keep_actions === FALSE) $this->actions = array();
		...
		switch($type){
			...
			case 'height':
				if(is_string($value) and !ctype_digit($value)){
					...
					if(!preg_match('/^[0-9]++%$/D', $value)) return FALSE;
				}else{
					$value = (int)$value;
				}
				break;
			case 'top':
				if(is_string($value) and !ctype_digit($value)){
					if(!in_array($value, array('top','bottom','center'))) return FALSE;
				}else{
					$value = (int)$value;
				}
				break;
			case 'left':
				if(is_string($value) and !ctype_digit($value)){
					if(!in_array($value, array('left','right','center'))) return FALSE;
				}else{
					$value = (int)$value;
				}
				break;
			case 'master':
				if($value !== Image::NONE and $value !== Image::AUTO and $value !== Image::WIDTH and $value !== Image::HEIGHT) return FALSE;
				break;
		}
