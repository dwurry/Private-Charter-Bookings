...
class Image_GD_Driver extends Image_Driver{
	...
	public function __construct(){
		...
		if(!function_exists('gd_info')) throw new CException('image gd requires v2');
		
		// Get the GD information
		$info = gd_info();
		
		// Make sure that the GD2 is installed
		if(strpos($info['GD Version'], '2.') === FALSE) throw new CException('image gd requires v2');
	}
	public function process($image, $actions, $dir, $file, $render = FALSE){
		...
		if(empty($create) or !function_exists($create)) throw new CException('image type not allowed');
		
		// Make sure the image type is supported for saving
		if(empty($save) or !function_exists($save)) throw new CException('image type not allowed');
	...
	public function sharpen($amount){
		...
		if(!function_exists('imageconvolution')) throw new CException('image unsupported method');
	...
	/**
	 * Returns an image with a transparent background.
	 * Used for rotating to
